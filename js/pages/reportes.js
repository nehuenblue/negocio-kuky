...
function renderTopDeudores(deudores) {
  ...
  $lista.innerHTML = deudores.map(d => {
    ...
    // Botón WhatsApp si tiene teléfono cargado
    let btnWsp = '';
    if (d.telefono) {
      const mensaje = TEMPLATES_WSP.recordatorioCobro({
        nombreCliente: nombre,
        saldoPendiente: d.saldoPendiente,
      });
      const link = generarLinkWhatsApp(d.telefono, mensaje);
      if (link) {
        btnWsp = `
          <a href="${link}" target="_blank" rel="noopener" class="btn-wsp-mini" title="Mandar recordatorio por WhatsApp">
